...
pub mod win_memory {
    ...
    pub fn find_pid_by_name(process_name: &str) -> std::io::Result<u32> {
        ...
        let snapshot = unsafe { CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0) }
            .map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, format!("Failed to create process snapshot: {}", e)))?;
        ...
        pid.ok_or_else(|| std::io::Error::new(std::io::ErrorKind::NotFound, format!("Process not found: {}", process_name)))
    }
    ...
    impl ProcessInstance {
        ...
        pub fn write_memory(&self, offset: usize, data: &[u8]) -> std::io::Result<()> {
            ...
            match result {
                ...
                Ok(_) => Err(std::io::Error::new(std::io::ErrorKind::Other, format!("Only wrote {} bytes", bytes_written))),
                Err(e) => Err(std::io::Error::from_raw_os_error(e.code().0 as i32)),
            }
        }
        /// Write u32 at offset
